...
ASOF = "2026-08-11"          # 口径日期 (G0d 过热保险丝接线后)
...
# ── 回测业绩 (百分比口径) ──
st.header("回测业绩（全栈模拟：信号→过滤器→分钟级执行→成本）")
...
- **regime 边界**：黑天鹅不可预测，本系统不声称能预测崩盘。入场侧有过热保护
  （2015/2007 顶部区样本外重放全挡），持仓侧**不做**崩盘保护——用可承受的回撤买崩盘保险
  （全时段 -20.84% vs 2019 口径 -2.59% 的差额即保费）。
- **回测≠实盘**：回测含分钟级执行模拟与全部交易成本；实盘为纸交易（无真实滑点冲击），
  两者存在执行折价，按季度归因公布。
- **永不使用杠杆**。
""")
